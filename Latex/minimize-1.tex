...
split $Q_0$ wrt ($Q_1,0$), see Fig. \ref{fig:split}.
...
$\therefore$
$Q_0$ wrt ($Q_1,0$) is splitted into two parts. part(1) $Q_0^\prime=\{4\}$, part(2) $Q_0^{\prime\prime}=Q_0\setminus Q_0^\prime=\{0,1,2,3\}$ 
\newline
...
$\therefore$
$Q_0$ wrt ($Q_1,0$) 是一个无效的split, 同样$Q_0=Q_1=\{5\}$也是一个无效的split。
\newline
